...
class MLP(nn.Module):
    def __init__(self, length):
        ...
        self.length = length
        self.a1 = nn.Parameter(torch.zeros(1, self.length))
        self.a2 = nn.Parameter(torch.zeros(1, self.length))
        self.a3 = nn.Parameter(torch.ones(1, self.length))
        self.a4 = nn.Parameter(torch.ones(1, self.length))
        self.a4 = nn.Parameter(torch.zeros(1, self.length))
        self.a6 = nn.Parameter(torch.zeros(1, self.length))
        self.a7 = nn.Parameter(torch.zeros(1, self.length))
        self.a8 = nn.Parameter(torch.zeros(1, self.length))
        self.a9 = nn.Parameter(torch.ones(1, self.length))
        self.a10 = nn.Parameter(torch.ones(1, self.length))

    def forward(self, zl, ul):
        u = self.a1 * F.sigmoid(self.a2*ul + self.a3) + self.a4*ul + self.a5
        v = self.a6 * F.sigmoid(self.a7*ul + self.a8) + self.a9*ul + self.a10
        out = (zl - u)*v + u
        return out
...
class Combinator(nn.Module):
    def __init__(self, length):
        ...
        self.length = length
        self.bias_0 = nn.Parameter(torch.zeros(1, self.length))
        self.bias_1 = nn.Parameter(torch.zeros(1, self.length))
        self.w_0z = nn.Parameter(torch.ones(1, self.length))
        self.w_1z = nn.Parameter(torch.ones(1, self.length))
        self.w_0u = nn.Parameter(torch.zeros(1, self.length))
        self.w_1u = nn.Parameter(torch.zeros(1, self.length))
        self.w_0zu = nn.Parameter(torch.zeros(1, self.length))
        self.w_1zu = nn.Parameter(torch.zeros(1, self.length))
        self.w_sig = nn.Parameter(torch.ones(1, self.length))
...
class Ladder(nn.Module):
    ...
    def addgaussianNoise(self, x):
        x.data = x.data + torch.Tensor(np.random.normal(c.MEAN, c.STD, x.size()))
        return x
    ...
    def clean_encoder(self, x):
        x = x.view(-1, 784)
        self.batch_mean.append(x.mean())
        self.batch_std.append(x.std())
        y = x.clone().detach()
        y = self.abn0(y)
    ...
    def bnormalize(self, inp, mean, std):
        # return inp
        inp.data = inp.data - mean.data.repeat(inp.size(0), inp.size(1))
        inp.data = inp.data / (std.data.repeat(inp.size(0), inp.size(1)) + 1e-5)
        return inp
    ...
    def decoder(self, x):
        ...
        self.decoder_out[6] = self.bnormalize(x.clone(), self.batch_mean[6], self.batch_std[6])

        x = self.tfc6(x)
        x = F.relu(x)
        x = self.abn5(x)
        x = self.comb5.forward(self.layer_out[5], x)
        self.decoder_out[5] = self.bnormalize(x.clone(), self.batch_mean[5], self.batch_std[5])

        x = self.tfc5(x)
        x = F.relu(x)
        x = self.abn4(x)
        x = self.comb4.forward(self.layer_out[4], x)
        self.decoder_out[4] = self.bnormalize(x.clone(), self.batch_mean[4], self.batch_std[4])

        x = self.tfc4(x)
        x = F.relu(x)
        x = self.abn3(x)
        x = self.comb3.forward(self.layer_out[3], x)
        self.decoder_out[3] = self.bnormalize(x.clone(), self.batch_mean[3], self.batch_std[3])

        x = self.tfc3(x)
        x = F.relu(x)
        x = self.abn2(x)
        x = self.comb2.forward(self.layer_out[2], x)
        self.decoder_out[2] = self.bnormalize(x.clone(), self.batch_mean[2], self.batch_std[2])

        x = self.tfc2(x)
        x = F.relu(x)
        x = self.abn1(x)
        x = self.comb1.forward(self.layer_out[1], x)
        self.decoder_out[1] = self.bnormalize(x.clone(), self.batch_mean[1], self.batch_std[1])

        x = self.tfc1(x)
        x = F.relu(x)
        x = self.comb0.forward(self.layer_out[0], x)
        self.decoder_out[0] = self.bnormalize(x, self.batch_mean[0], self.batch_std[0])
